...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python server_live.py <video_path1> [<video_path2> ...] [video_port] [audio_port] [control_port]")
        sys.exit(1)
    ...
    control_port = int(sys.argv[idx+2]) if idx+2 < len(sys.argv) else video_port + 10
    ...
    server = LiveStreamServer(video_paths, video_port=video_port, audio_port=audio_port, control_port=control_port)
    try:
        server.start()
